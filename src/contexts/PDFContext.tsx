...
export const PDFProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const handlePreAlertPDF = async (shipmentId: string, options?: PdfGenerationOptions) => {
    try {
      ...
      // Add default logo if not provided - always ensure the logo is included
      const pdfOptions = {
        ...options,
        logoImgData: options?.logoImgData || DEFAULT_LOGO,
      };
      
      console.log("PDFContext: Logo type for Pre-Alert PDF:", 
        typeof pdfOptions.logoImgData, 
        pdfOptions.logoImgData ? "Logo provided (length: " + pdfOptions.logoImgData.substring(0, 30) + "...)" : "No logo");
  ...
  const handleCMRPDF = async (shipmentId: string, options?: PdfGenerationOptions) => {
    try {
      ...
      console.log("PDFContext: Logo type for CMR PDF:", 
        typeof pdfOptions.logoImgData, 
        pdfOptions.logoImgData ? "Logo provided (length: " + pdfOptions.logoImgData.substring(0, 30) + "...)" : "No logo");
